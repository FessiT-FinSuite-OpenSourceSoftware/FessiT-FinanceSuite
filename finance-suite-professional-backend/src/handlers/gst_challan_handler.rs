...
use crate::{
    ...
    services::gst_challan_service::GstChallanService,
    utils::auth::Claims,
    utils::permissions::{check_permission, Module, PermissionAction, create_permission_error},
};

const UPLOAD_DIR: &str = "./uploads/gst_challans";

fn parse_f64(s: &str) -> f64 {
    s.parse::<f64>().unwrap_or(0.0)
}

/// POST /api/v1/gst-challans  (multipart — includes optional file)
#[post("/gst-challans")]
pub async fn create_gst_challan(
    service: web::Data<GstChallanService>,
    ...
    let saved = service.create(gst_challan, &org_id).await
        .map_err(actix_web::error::ErrorInternalServerError)?;
    Ok(HttpResponse::Created().json(saved))
}

/// GET /api/v1/gst-challans
#[get("/gst-challans")]
pub async fn list_gst_challans(
    service: web::Data<GstChallanService>,
    http_req: HttpRequest,
) -> actix_web::Result<impl Responder> {
    let claims = http_req.extensions().get::<Claims>().cloned()
    ...
    let org_id = user.organisation_id
        .ok_or_else(|| actix_web::error::ErrorBadRequest("User has no organisation"))?;
    let gst_challans = service.list(&org_id).await
        .map_err(actix_web::error::ErrorInternalServerError)?;
    Ok(HttpResponse::Ok().json(gst_challans))
}

/// GET /api/v1/gst-challans/{id}
#[get("/gst-challans/{id}")]
pub async fn get_gst_challan(
    service: web::Data<GstChallanService>,
    id: Path<String>,
    http_req: HttpRequest,
) -> actix_web::Result<impl Responder> {
        ...
        .map_err(actix_web::error::ErrorInternalServerError)?
    {
        ...
        _ => Ok(HttpResponse::NotFound().json(json!({ "message": "GST Challan not found" }))),
    }
}

/// PUT /api/v1/gst-challans/{id}  (multipart — includes optional new file)
#[put("/gst-challans/{id}")]
pub async fn update_gst_challan(
    service: web::Data<GstChallanService>,
    id: Path<String>,
    mut payload: Multipart,
    http_req: HttpRequest,
    ...
    let existing = service.get_by_id(&id).await
        .map_err(actix_web::error::ErrorInternalServerError)?
        .ok_or_else(|| actix_web::error::ErrorNotFound(json!({ "message": "GST Challan not found" })))?;
        ...
        .map_err(actix_web::error::ErrorInternalServerError)?
    {
        ...
        None => Ok(HttpResponse::NotFound().json(json!({ "message": "GST Challan not found" }))),
    }
}

/// DELETE /api/v1/gst-challans/{id}
#[delete("/gst-challans/{id}")]
pub async fn delete_gst_challan(
    service: web::Data<GstChallanService>,
    id: Path<String>,
    http_req: HttpRequest,
) -> actix_web::Result<impl Responder> {
    ...
    let existing = service.get_by_id(&id).await
        .map_err(actix_web::error::ErrorInternalServerError)?;
    match existing {
        None => return Ok(HttpResponse::NotFound().json(json!({ "message": "GST Challan not found" }))),
    ...
    if deleted {
        ...
        Ok(HttpResponse::NotFound().json(json!({ "message": "GST Challan not found" })))
    }
}

/// GET /api/v1/gst-challan-files/{filename}  — serve the uploaded GST challan file
#[get("/gst-challan-files/{filename}")]
pub async fn get_gst_challan_file(
    path: Path<String>,
    service: web::Data<GstChallanService>,
    http_req: HttpRequest,
) -> actix_web::Result<impl Responder> {
    ...
    let gst_challans = service.list(&org_id).await
        .map_err(actix_web::error::ErrorInternalServerError)?;
    if !gst_challans.iter().any(|c| c.file == filename) {
...
pub fn configure_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(get_gst_challan_file)
        .service(create_gst_challan)
        .service(list_gst_challans)
        .service(get_gst_challan)
        .service(update_gst_challan)
        .service(delete_gst_challan);
}
