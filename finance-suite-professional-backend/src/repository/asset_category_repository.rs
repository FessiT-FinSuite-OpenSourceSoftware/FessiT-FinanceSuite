...
use crate::models::asset_category::AssetCategory;

#[derive(Clone)]
pub struct AssetCategoryRepository {
    collection: Arc<Collection<AssetCategory>>,
    raw_collection: Arc<Collection<Document>>,
}

fn doc_to_asset_category(raw: Document) -> Option<AssetCategory> {
    let oid = raw.get_object_id("_id").ok();
    let mut cat: AssetCategory = from_document(raw).ok()?;
    cat.id = oid;
    Some(cat)
}

impl AssetCategoryRepository {
    pub fn new(collection: Collection<AssetCategory>) -> Self {
    ...
    async fn fetch_many(&self, filter: impl Into<Option<Document>>) -> Result<Vec<AssetCategory>, MongoError> {
        let mut cursor = self.raw_collection.find(filter, None).await?;
        let mut list = Vec::new();
        while let Some(raw) = cursor.try_next().await.unwrap_or(None) {
            if let Some(c) = doc_to_asset_category(raw) {
                list.push(c);
            }
        }
        Ok(list)
    ...
    pub async fn create(&self, mut category: AssetCategory) -> Result<AssetCategory, MongoError> {
    ...
    pub async fn get_by_org(&self, org_id: &ObjectId) -> Result<Vec<AssetCategory>, MongoError> {
        self.fetch_many(doc! { "organisationId": org_id }).await
    }

    pub async fn get_by_id(&self, id: &str) -> Result<Option<AssetCategory>, MongoError> {
        let oid = match ObjectId::parse_str(id) {
            Ok(o) => o,
            Err(_) => return Ok(None),
        };
        let raw = self.raw_collection.find_one(doc! { "_id": oid }, None).await?;
        Ok(raw.and_then(doc_to_asset_category))
    }

    pub async fn update(&self, id: &str, mut category: AssetCategory) -> Result<Option<AssetCategory>, MongoError> {
        let oid = match ObjectId::parse_str(id) {
            Ok(o) => o,
            Err(_) => return Ok(None),
        };
