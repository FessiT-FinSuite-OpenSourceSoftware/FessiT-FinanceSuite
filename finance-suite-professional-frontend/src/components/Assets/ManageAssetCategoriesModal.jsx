...
export default function ManageAssetCategoriesModal({ onClose }) {
  const dispatch = useDispatch()
  const { assetCategoryData, isLoading } = useSelector(assetCategorySelector)
  ...
  const handleUpdate = async (id) => {
    ...
    await dispatch(updateAssetCategory(id, editValue.trim()))
    setEditingId(null)
    setEditValue('')
  }
  ...
  const handleAdd = async () => {
    ...
    await dispatch(createAssetCategory(newName.trim()))
    setNewName('')
  }
      ...
      <div className="w-full max-w-md flex flex-col h-[60vh] rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-slate-900">Manage Asset Categories</h3>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-3 space-y-1">
          {isLoading && categories.length === 0 ? (
            ...
            <p className="text-sm text-slate-400 text-center py-6">No asset categories yet.</p>
          ) : categories.map((cat) => {
              ...
              <div key={id} className="flex items-center gap-2 rounded-xl  py-1 px-0.5 hover:bg-slate-50 group">
                {isEditing ? (
                  ...
                  <>
                    ...
                    <button onClick={() => handleDelete(id)} className="text-xs font-semibold text-white bg-red-600 hover:bg-red-700 rounded-lg px-2 py-1">Yes</button>
                    <button onClick={() => setDeleteConfirmId(null)} className="text-xs font-semibold text-slate-600 border border-slate-300 rounded-lg px-2 py-1 hover:bg-slate-50">No</button>
                  </>
                ) : (
                  <>
