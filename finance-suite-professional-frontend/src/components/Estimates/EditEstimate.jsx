...
import { fetchEstimateById, updateEstimate, estimateSelector } from "../../ReduxApi/estimate";
...
const emptyItem = () => ({ name: "", description: "", isManual: false, quantity: "", unitPrice: "", discount: "", taxRate: "", amount: 0 });
...
export default function EditEstimate() {
  ...
  const { currentEstimate, isLoading } = useSelector(estimateSelector);
  const { productData }   = useSelector(productSelector);
  const { customersData } = useSelector(customerSelector);
  const { currentOrganisation } = useSelector(orgamisationSelector);

  const [form, setForm] = useState({ customerId: "", issueDate: "", expiryDate: "", currency: "INR", discount: "", notes: "", terms: "" });
  ...
  useEffect(() => {
    ...
    setEstimateNumber(e.estimateNumber || e.estimate_number || "");
    setForm({
      customerId: e.customerId || e.customer_id || "",
      issueDate:  e.issueDate  || e.issue_date || "",
      expiryDate: e.expiryDate || e.expiry_date || "",
      currency:   e.currency   || e.currency_type || "INR",
    ...
    setItemSearchQueries((e.items || []).map(() => ""));
    // pre-fill customer search label
    const cust = (Array.isArray(customersData) ? customersData : []).find((c) => getCustomerId(c) === (e.customerId || e.customer_id));
    setCustomerSearch(cust?.companyName || cust?.customerName || e.customerName || "");
  }, [currentEstimate, customersData]);
  ...
  const handleProductSelect = (idx, product) => {
    setItems((prev) => prev.map((item, i) => {
      ...
      const next = { ...item, name: product.name || "", unitPrice: String(product.sale_price ?? product.salePrice ?? ""), isManual: false };
      next.amount = calcAmount(next);
      return next;
    }));
  ...
  const addItem    = () => { setItems((p) => [...p, emptyItem()]); setItemSearchQueries((p) => [...p, ""]); };
  const removeItem = (idx) => { setItems((p) => p.filter((_, i) => i !== idx)); setItemSearchQueries((p) => p.filter((_, i) => i !== idx)); };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.issueDate)  { toast.error("Issue date is required"); return; }
    if (!form.currency)   { toast.error("Currency is required"); return; }
    if (!items.length)    { toast.error("At least one item is required"); return; }
    for (const it of items) {
      if (!it.name.trim())                  { toast.error("Each item must have a name"); return; }
      if (!(parseFloat(it.quantity) > 0))   { toast.error("Item quantity must be > 0"); return; }
      if (parseFloat(it.unitPrice) < 0)     { toast.error("Unit price must be ≥ 0"); return; }
    }
    const payload = {
      estimateNumber,
    ...
    setSaving(true);
    try { await dispatch(updateEstimate(id, payload)); nav("/estimates"); }
  ...
  const previewData = {
    ...
    items: items.map((it) => ({ name: it.name, description: it.description || "", quantity: parseFloat(it.quantity) || 0, unitPrice: parseFloat(it.unitPrice) || 0, discount: it.discount !== "" ? parseFloat(it.discount) : null, taxRate: it.taxRate !== "" ? parseFloat(it.taxRate) : null, amount: it.amount })),
  };
      ...
      <div className="sticky top-[88px] w-full z-30 rounded-lg bg-white border border-gray-300 py-4 -mt-15 shadow-sm">
        <div className="flex justify-between px-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-gray-700">Edit Estimate</span>
          </div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 pb-6 mt-5">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div>
            ...
            <input value={estimateNumber} disabled className="border border-gray-300 rounded px-3 py-2 w-full text-sm bg-gray-50 cursor-not-allowed text-gray-700" />
          </div>
          <div>
        ...
        <div className="mb-6">
          ...
          <div className="relative w-full sm:w-1/2 lg:w-1/3">
            <input type="text" placeholder="Search customer..." className="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-700 placeholder:text-gray-400"
            ...
            {items.map((item, idx) => (
              <tr key={idx} className="hover:bg-gray-50 transition">
                ...
                <td className="border border-gray-300 px-3 py-2 min-w-[220px]">
                  {!item.isManual ? (
                    <div className="relative">
                      <input type="text" className="w-full border border-gray-300 rounded px-2 py-1 text-sm text-gray-700 placeholder:text-gray-400" placeholder="Search or type item..."
                        value={item.name && !itemSearchQueries[idx] ? item.name : (itemSearchQueries[idx] ?? "")}
                        onChange={(e) => { const q = [...itemSearchQueries]; q[idx] = e.target.value; setItemSearchQueries(q); if (!e.target.value) handleProductSelect(idx, ""); }}
                        onFocus={() => { const q = [...itemSearchQueries]; q[idx] = item.name || ""; setItemSearchQueries(q); }}
                        onBlur={() => { const typed = itemSearchQueries[idx]; if (typed?.trim()) setItems((prev) => prev.map((it, i) => i === idx ? { ...it, name: typed.trim() } : it)); const q = [...itemSearchQueries]; q[idx] = ""; setItemSearchQueries(q); }}
                      />
                      {itemSearchQueries[idx] && (() => {
                        const q = itemSearchQueries[idx].toLowerCase();
                        const filtered = (Array.isArray(productData) ? productData : []).filter((p) => (p.name || "").toLowerCase().includes(q) || (p.hsn || "").toLowerCase().includes(q));
                        return (
                          <div className="absolute z-50 top-full left-0 w-full bg-white border border-gray-300 rounded shadow-lg max-h-48 overflow-y-auto">
                  ...
                  <input type="text" name="description" value={item.description} onChange={(e) => handleItemChange(idx, e)} className="w-full border border-gray-200 rounded px-2 py-1 text-xs text-gray-500 mt-1" placeholder="Description (optional)" />
                </td>
                ...
                <td className="border border-gray-300 px-3 py-2 text-right font-semibold text-gray-800">{item.amount.toFixed(2)}</td>
                <td className="border border-gray-300 px-3 py-2 text-center">
                  {idx === items.length - 1
