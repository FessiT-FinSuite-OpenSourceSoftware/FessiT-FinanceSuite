...
export default function SalaryTab() {
  ...
  const columns = [
    { label: "Emp ID",   render: (r) => <span className="text-gray-600">{r.emp_id}</span> },
    { label: "Employee", render: (r) => <span className="font-medium text-gray-900">{r.emp_name}</span> },
    { label: "Period",   render: (r) => <span className="text-gray-600">{r.period ? new Date(r.period + "-01").toLocaleDateString("en-IN", { month: "long", year: "numeric" }) : "-"}</span> },
    { label: "Net Salary (₹)", render: (r) => <span className="font-semibold">₹ {fmt(r.net_salary)}</span> },
    {
      label: "Status",
      render: (r) => {
        ...
        return (
          <span
            onClick={() => !locked && setStatusPopup({ id: getId(r), status: r.status, paid_on: r.paid_on || "" })}
            className={`px-2 py-1 rounded-full text-xs font-medium transition-opacity ${statusColor(r.status)} ${!locked ? "cursor-pointer hover:opacity-75" : "cursor-default"}`}
          >
    ...
    { label: "Paid On", render: (r) => <span className="text-gray-600">{r.paid_on ? formatDate(r.paid_on) : "-"}</span> },
    {
      label: "Actions",
      right: true,
