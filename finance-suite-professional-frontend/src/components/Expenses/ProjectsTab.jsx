...
export default function ProjectsTab() {
  ...
  const handleSave = () => {
    ...
    if (modal.mode === "create") setRows((p) => [{ ...payload, id: Date.now() }, ...p]);
    else setRows((p) => p.map((r) => r.id === modal.id ? { ...payload, id: modal.id } : r));
    closeModal();
  };
  ...
  const columns = [
    { label: "Project Name", render: (r) => <span className="font-medium text-gray-900">{r.name}</span> },
    { label: "Manager",      render: (r) => <span className="text-gray-600">{r.manager}</span> },
    { label: "Budget",       render: (r) => <span>₹ {r.budget.toLocaleString("en-IN")}</span> },
    {
      label: "Spent",
      render: (r) => {
    ...
    },
    {
      ...
      render: (r) => {
        ...
        return <span className={`font-semibold ${remaining < 0 ? "text-red-600" : "text-green-600"}`}>₹ {remaining.toLocaleString("en-IN")}</span>;
      },
    },
    { label: "Deadline", render: (r) => <span className="text-gray-600">{r.deadline}</span> },
    { label: "Status",   render: (r) => <StatusBadge status={r.status} colorFn={statusColor} /> },
    {
      label: "Actions",
      right: true,
  ...
  return (
    <div className="max-w-7xl lg:w-full md:w-full">
      ...
      <DataTable data={filtered} rowKey={(r) => r.id} columns={columns} />

      {modal && (
        <Modal title={modal.mode === "create" ? "Add Project" : "Edit Project"} onClose={closeModal} onSave={handleSave}>
          <FormField label="Project Name" colSpan><input type="text" name="name" value={form.name} onChange={handleChange} className={inputCls} /></FormField>
          {[{ label: "Manager", name: "manager", type: "text" }, { label: "Deadline", name: "deadline", type: "date" }, { label: "Budget (₹)", name: "budget", type: "number" }, { label: "Spent (₹)", name: "spent", type: "number" }].map(({ label, name, type }) => (
            <FormField key={name} label={label}><input type={type} name={name} value={form[name]} onChange={handleChange} className={inputCls} /></FormField>
          ))}
