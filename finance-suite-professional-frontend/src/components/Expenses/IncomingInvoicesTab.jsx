...
export default function IncomingInvoicesTab() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (bill) => {
        const bid = getIncomingId(bill);
        const locked = bill.status === "Paid" && !isAdmin;
        return (
          <span
            onClick={() => !locked && hasWrite && setStatusPopup({ id: bid, status: bill.status || "Unpaid", paidDate: bill.paid_date || "", paymentType: bill.payment_type || "", paymentReference: bill.payment_reference || "" })}
            className={!locked && hasWrite ? "cursor-pointer hover:opacity-75" : "cursor-default"}
          >
    ...
    {
      label: "Paid Date",
      render: (bill) => <span className="text-sm text-gray-700">{bill.status === "Paid" && bill.paid_date ? formatDate(bill.paid_date) : "—"}</span>,
    },
    {
      label: "Actions",
      right: true,
      ...
      render: (bill) => {
        ...
        return (
          <div className="flex items-center justify-end gap-2">
            {bill.status === "Paid" && (
              <button onClick={() => setPaymentDetail(bill)} title="Payment Details" className="hover:text-green-800 transition-colors">
  ...
  return (
    <div className="max-w-7xl lg:w-full md:w-full">
      <TabActionBar searchValue={search} onSearchChange={(v) => { setSearch(v); setPage(1); }} searchPlaceholder="Search by bill number or vendor...">
        ...
        {hasWrite && (
          <div className="relative">
            <button onClick={() => setShowTypeMenu((v) => !v)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-sm">
      ...
      <DataTable isLoading={isLoading} data={current} rowKey={(bill) => getIncomingId(bill) || bill.invoice_number} columns={columns} />
            ...
            {statusPopup.status === "Paid" && (
              <div className="mt-3 space-y-3">
                <div>
                  ...
                  <input type="date" value={statusPopup.paidDate || ""} onChange={(e) => setStatusPopup((p) => ({ ...p, paidDate: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Payment Type *</label>
                  <select value={statusPopup.paymentType || ""} onChange={(e) => setStatusPopup((p) => ({ ...p, paymentType: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                  ...
                  <input type="text" placeholder="UTR / Cheque no. / Transaction ID" value={statusPopup.paymentReference || ""} onChange={(e) => setStatusPopup((p) => ({ ...p, paymentReference: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 w-80">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm"><span className="text-gray-500">Invoice No</span><span className="font-medium text-gray-800">{paymentDetail.invoice_number || "—"}</span></div>
              ...
              <div className="border-t border-gray-100 pt-3 space-y-3">
                ...
                <div className="flex justify-between text-sm"><span className="text-gray-500">Payment Type</span><span className="font-medium text-gray-800">{paymentDetail.payment_type || "—"}</span></div>
                <div className="flex justify-between text-sm"><span className="text-gray-500">Reference</span><span className="font-medium text-indigo-600 font-mono text-xs">{paymentDetail.payment_reference || "—"}</span></div>
              </div>
            </div>
