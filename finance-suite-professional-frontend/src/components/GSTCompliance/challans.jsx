...
const CHALLAN_FILE_ENDPOINTS = ["/challan-files", "/gst-challan-files"];
...
export default function ChallansTab() {
  ...
  const { gstchallanData, isLoading } = useSelector(challanSelector);
  const challanData = gstchallanData;
  ...
  const fetchChallanFile = async (filename, responseType) => {
    let lastError;
    for (const endpoint of CHALLAN_FILE_ENDPOINTS) {
      try {
        return await axiosInstance.get(`${endpoint}/${encodeURIComponent(filename)}`, { responseType });
      } catch (error) {
        lastError = error;
  ...
  const handleViewDoc = async (filename) => {
    ...
    try {
      const res = await fetchChallanFile(filename, "blob");
      ...
      console.error("GST challan preview error:", error);
  ...
  const handleDownloadDoc = async (filename) => {
    ...
    try {
      ...
      let filePath = null;
      if (isTauri()) {
        const res = await fetchChallanFile(filename, "arraybuffer");
      ...
      console.error("GST challan download error:", error);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modal.mode === "create") {
        await dispatch(creategstChallan(payload));
      } else {
        await dispatch(updategstChallan(modal.id, payload));
      }
      closeModal();
    } catch {
  ...
  const handleDelete = () => {
    ...
    dispatch(deletegstChallan(deleteModal.id));
    setDeleteModal(null);
  };
